...
server.tool(
  "getInterceptData",
  "获取抓包概览列表（捕获的网络请求，每条仅含关键字段，不含 body）。支持按 host/path/app/method/状态码/时间窗口过滤。需要完整内容时用返回的 id 调用 getRequestDetail。",
  {
    url: z
      .string()
      .optional()
