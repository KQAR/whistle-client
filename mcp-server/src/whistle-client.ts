...
export class WhistleClient {
   ...
   */
  private async fetchDataPage(options: {
    startTime?: string;
    count?: number;
  }): Promise<{ items: Record<string, any>; lastId?: string; endId?: string }> {
    const now = Date.now();
  ...
  async getInterceptData(
  ...
  ): Promise<{ total: number; scanned: number; summaries: any[] }> {
    ...
    if (options.startTime) {
      ...
      while (scanned < scanLimit) {
        ...
        if (endId === undefined) endId = page.endId; // 记录起始时缓冲区的最新 id
        if (ids.length === 0) break;
        for (const id of ids) {
          scanned++;
          const summary = this.toSummary(page.items[id]);
          if (this.matchSummary(summary, options)) matched.push(summary);
        }
        const lastSeen = ids[ids.length - 1];
        if (endId && lastSeen === endId) break; // 已追上最新
        cursor = lastSeen;
        if (ids.length < 100) break; // 不足一页说明已到末尾
      }
    } else {
   ...
   */
  private parseUrl(url: string, fallbackHost?: string): { host: string; path: string } {
    if (!url) return { host: fallbackHost || "", path: "" };
  ...
  private toSummary(item: any): any {
    if (!item) return null;
    const req = item.req || {};
    const res = item.res || {};
    ...
    const cost =
      item.endTime && item.startTime ? item.endTime - item.startTime : undefined;
    return {
      id: item.id,
      url: item.url,
      host,
  ...
  private matchSummary(
    ...
    if (f.method && (s.method || "").toUpperCase() !== f.method.toUpperCase())
      return false;
    if (f.statusCode != null && s.statusCode !== f.statusCode) return false;
    if (f.startTime != null && (s.startTime || 0) < f.startTime) return false;
    if (f.endTime != null && (s.startTime || 0) > f.endTime) return false;
    return true;
  }
  ...
  async getRequestDetail(
    ...
    if (!item) {
      throw new Error(
        `Request '${id}' not found (可能已被缓冲区淘汰，请用 getInterceptData 重新获取最新 id)`
      );
    }
  ...
  private decodeBody(
    ...
    if (!b64) return { text: "", encoding: "none", truncated: false };
    let buf: Buffer;
    try {
    ...
    const ce = (
      (part.headers && part.headers["content-encoding"]) ||
      ""
    ).toLowerCase();
    try {
      if (ce.includes("br")) buf = brotliDecompressSync(buf);
      else if (ce.includes("gzip")) buf = gunzipSync(buf);
    ...
    const looksText =
      /text\/|json|xml|javascript|x-www-form-urlencoded|html|csv/.test(ct) ||
  ...
  private isProbablyText(buf: Buffer): boolean {
