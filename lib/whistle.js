...
const getShadowRules = (settings) => {
  ...
  const auth = username || password ? `* whistle.proxyauth://${username}:${password} %proxyauth.enableHash=true` : '';
...
const baseOptions = {
  ...
  mode: `client|disableUpdateTips|disableAuthUI${newOptions.rejectUnauthorized ? '|rejectUnauthorized' : ''}`,
  ...newOptions,
...
const proxy = startWhistle({
  ...
  try {
    const auth = Buffer.from(`${newOptions.username || ''}:${newOptions.password || ''}`).toString('base64');
    fs.writeFileSync(PROC_PATH, JSON.stringify({
      pid: process.pid, host, port, auth,
    }));
  } catch (e) {}
  let timer;
  let changeTimer;
